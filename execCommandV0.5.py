...
usage = "usage: %prog [options] args\n python execCommandV0.5.py -d CLIENT -T HOSTS_TBL -U root -P 'Super Secret' -D ROUTER commands.txt\n python execCommandV0.5.py -d CLIENT -U root -P 'Super Secret' -S \"select * from HOSTS_TBL where HOSTNAME like '%BOB%' AND DEVICE_TYPE='ROUTER'\" commands.txt"
...
for row in dbCursor.fetchall():
	print(row[5])
	with settings(hide('warnings', 'running', 'stdout', 'stderr'), warn_only = True, abort_on_promts = True, no_agent = True, no_keys = True, skip_bad_hosts = True, host_string = row[5], user = row[7], password = base64.b64decode(row[8])):
		try:
			for c in commands:
